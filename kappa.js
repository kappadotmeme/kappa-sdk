...
const updateTemplate = async (tokenData) => {
    ...
    try {
        ...
        if (typeof window !== 'undefined' && wasm.initWasm && !wasm.__wasm) {
            ...
            // Try to use the wasm-loader module first
            let wasmBytes;
            try {
                const wasmLoader = require('./src/wasm-loader.js');
                console.log('[kappa.js] Using wasm-loader module');
                wasmBytes = await wasmLoader.loadWasmBytes();
            } catch (e) {
                console.log('[kappa.js] wasm-loader not available, using fallback');
                // Fallback to inline loader if module not available
